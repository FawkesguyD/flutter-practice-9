...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onCheckAccount(
    ...
    try {
      ...
      LoggerService.info(
        'AuthBloc: Проверка аккаунта - ${hasAccount ? "есть" : "нет"}',
      );
    } catch (e) {
    ...
    try {
      ...
      if (success) {
        ...
        LoggerService.warning(
          'AuthBloc: Неудачная попытка входа для ${event.username}',
        );
  ...
  Future<void> _onRegisterRequested(
    ...
    try {
      ...
      if (success) {
        LoggerService.info(
          'AuthBloc: Успешная регистрация для ${event.username}',
        );

        // Синхронизируем имя профиля с именем регистрации
        try {
          final profile = await Services.profile.getProfile();
          final current = profile.nickname.trim();
          if (current.isEmpty || current == 'Аккаунт' || current == 'Повар') {
            await Services.profile.updateNickname(event.username.trim());
          }
        } catch (_) {}
        ...
        LoggerService.warning(
          'AuthBloc: Неудачная регистрация для ${event.username}',
        );
