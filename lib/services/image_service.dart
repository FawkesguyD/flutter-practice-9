...
class ImageService {
  ...
  static const String _foodTags = 'habit,checklist,calendar,fitness,health';
  ...
  bool _migrateLegacyPicsum() {
    ...
    LoggerService.warning(
      'ImageService: найдены старые picsum-ссылки, выполняю миграцию...',
    );
  ...
  String _avatarUrl(int seed) => 'https://i.pravatar.cc/$_avatarSize?u=$seed';
  ...
  Future<void> preloadImagePool() async {
    ...
    for (final url in _availableImages) {
      try {
        ...
        LoggerService.debug(
          'Предзагружено изображение карточки привычки: $url',
        );
      } catch (e) {
    ...
    if (_availableImages.isEmpty) {
      ...
      _preloadImages(_availableImages).catchError(
        (e) => LoggerService.warning(
          'Не удалось предзагрузить новые изображения: $e',
        ),
      );
    }
  ...
  Future<String?> getNextAvatar() async {
    ...
    if (_availableAvatars.isEmpty) {
      ...
      _preloadImages(_availableAvatars).catchError(
        (e) => LoggerService.warning(
          'Не удалось предзагрузить новые аватарки: $e',
        ),
      );
    }
  ...
  String? assetForGenre(String genre, {String? title}) {
    final g = genre.toLowerCase();
    final t = (title ?? '').toLowerCase();
    if (t.contains('вода') || g.contains('вода') || g.contains('питание')) {
      return 'assets/water.jpeg';
    }
    if (g.contains('здоров')) {
      return 'assets/health.jpeg';
    }
    if (g.contains('фитнес') || g.contains('спорт')) {
